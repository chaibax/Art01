        ...
        axios.get(process.env.REACT_APP_API_BASE_URL+'/pixels/squaresize', {
            ...
            .then(function (response) {
              ...
              if( (response.data.squaresize) ){
                console.log(response.data.squaresize);
 ...
 getNewLatticeCoordinatesFor  = function(position, squaresize) {
    ...
    return [result_old[0]+modifx,newy]
      ...
      refresh(size){
        ...
        var self = this;
        console.log('refresh! avec size ='+size);
        this.setState(state => ({
      ...
      // get canvas context
        //var ctx = document.getElementById('canvart').getContext('2d');
        // load image
        var image = new Image();
        image.onload = function () {
           ...
           console.log(coordonne);
           console.log('position ='+self.state.position+ " size = "+size)
